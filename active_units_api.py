#!/usr/bin/env python3
"""Active Units API for Rdio-Scanner.

- Reads /home/lynx/Desktop/rdio-scanner.db
- Uses rdioScannerCalls + rdioScannerUnits (+ rdioScannerSystems) to expose:
    * unit label
    * system name
    * times seen in last hour
    * last seen time
- Only includes units with a non-empty label
...
def active_units():
    ...
    try:
        ...
        for row in rows:
            ...
            # Parse dateTime string to ISO 8601
            last_seen_iso = None
            if raw_last_seen:
                try:
